...
{
  auto res=lag::as_matrix(_mx,3,3);
  res(0,1)=res(1,0)=0;
...
void ScaleMx2D(matrix_t&_mx,
                ...
                const array_t&to2)
{
    using real_t=lag::vector_value_t<array_t>;
    auto mx=lag::as_matrix(_mx,3,3);
...
{
    auto res=lag::as_matrix(_mx,4,4);
    res.set_identity();
    res(0,3)=delta[0];
    res(1,3)=delta[1];
    res(2,3)=delta[2];
