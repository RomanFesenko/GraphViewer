...
class CFunctionalMesh
{
    ...
    std::function<void(const CFunctionalMesh&,CUpdateResult)> m_update_callback;
    ...
    template<class f_t>
    CFunctionalMesh& SetUpdateCallback(f_t f)
    {
        m_update_callback=f;
        return *this;
    }
    ...
    CFunctionalMesh& SetUniformColor(float x,float y,float z){return SetUniformColor(point_t(x,y,z));}
    ...
    CFunctionalMesh& SetTransparency(float t)
    {
        m_transparency=t;
        return *this;
    }
    ...
    const std::vector<level_line_t>*  Levels(int i)const;
