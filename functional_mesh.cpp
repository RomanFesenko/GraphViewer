...
// Lazy updating all hard evaluating matrix:
// m_points - only if change functor
// m_colors - only if change functor and plots colored, etc
...
CFunctionalMesh::CUpdateResult CFunctionalMesh::UpdateData(float time)
{
    ...
    if(m_functor_update)
    {
        m_functor_update=false;
        update|=CUpdateResult::update_functor;
    }
