...
static QTime glTime;
static CFunctionPool glFunctionPool;
...
static void SetCameraAndLight(CScene&scene)
{
  using point_t=Eigen::Vector3f;
  if(scene.Empty()) return;
  assert(glMesh.BoundedBox());
  point_t min_corner=glMesh.BoundedBox()->first;
  point_t max_corner=glMesh.BoundedBox()->second;


  point_t center=(min_corner+max_corner)/2.0f;
  point_t delta=max_corner-min_corner;
  float _l=0.5*(delta[0]+std::max(delta[1],delta[2])/ std::tan(scene.Camera().Aspect()/2.0));
  scene.Camera().SetPosition(center+point_t::UnitX()*_l);
  scene.Camera().SetView(-point_t::UnitX());

  scene.LightSource().SetPosition(center+point_t::UnitZ()*delta[2]);
}
...
ParseFloats(const CBaseDialog&dg,
            ...
            std::string &str,const CFunctionPool&pool)
{
  //const std::regex reg=std::regex("-?[[:digit:]]+(\\.[[:digit:]]*)?");
  std::vector<float> floats(*rgs::max_element(fields)+1);
  expr::parse_error_t error;
  for(int i:fields)
  {
      auto text=dg.Text(i,1);
      auto expr=pool.CreateFunction({},text,error);
      if(error)
      {
          str="For "+dg.Text(i,0)+" is not valid expression:"+error.detail();
          return {};
      }
      floats[i]=expr();
  }
...
void CMainWindow::m_LoadFromJson()
{
  ...
  auto err=FromJson(content,glFunctionPool);
  if(!err.empty())
  {
      QMessageBox::information(nullptr,
                               "JSON error",
...
void CMainWindow::m_SaveToJson()
{
  ...
  std::string  json_str;
  ToJson(json_str,glFunctionPool);
  file<<json_str;
}
...
void CMainWindow::m_3D_cartesian_edit()
{
  ...
  ->std::string
  {
      std::string mess;
      expr::parse_error_t error;
      auto floats=ParseFloats(dg,{1,2,3,4},mess,glFunctionPool);
      ...
      auto f=glFunctionPool.CreateFunction({"x","y","time"},dg.Text(0,1),error);
      if(error) return  "Error:"+error.detail();

      if(sutil::find_identifier(dg.Text(0,1),"time")==dg.Text(0,1).end())
      {
      ...
      glMesh.UpdateData(Now());
      SetCameraAndLight(ViewWidget()->Scene());
...
void CMainWindow::m_3D_spherical_edit()
{
  ...
  {
      expr::parse_error_t error;
      auto f=glFunctionPool.CreateFunction({"t","phi","time"},dg.Text(0,1),error);
      if(error) return  "Error:"+error.detail();
...
void CMainWindow::m_3D_polar_edit()
{
  ...
  ->std::string
  {
      expr::parse_error_t error;
      std::string str;
      auto floats=ParseFloats(dg,{1},str,glFunctionPool);
      if(!str.empty()) return str;
      if(floats[1]<=0)return "max r must be greater 0";

      auto f=glFunctionPool.CreateFunction({"r","phi","time"},dg.Text(0,1),error);
      if(error) return  "Error:"+error.detail();
      ...
      for(decltype(v_str.size()) i=0;i<v_str.size();++i)
      {
          m_previous_dg.m_edit[i]=dg.Text(i,1);
      }
...
void CMainWindow::m_3D_parametric_edit()
{
  ...
  for(decltype(v_str.size()) i=0;i<v_str.size();++i)
  {
      dg->SetText(i,1,m_previous_dg.m_edit[i]);
  }
  auto preprocess=[this,&v_str](const CBaseDialog&dg)
  ->std::string
  {
      expr::parse_error_t error;
      std::string str;
      auto floats=ParseFloats(dg,{3,4,5,6},str,glFunctionPool);
      if(!str.empty()) return str;
      if(floats[3]>=floats[4])return "min s must be less max s";
      if(floats[5]>=floats[6])return "min t must be less max t";
      auto _x=glFunctionPool.CreateFunction({"s","t","time"},dg.Text(0,1),error);
      if(error) return  "Error in x(...):"+error.detail();
      auto _y=glFunctionPool.CreateFunction({"s","t","time"},dg.Text(1,1),error);
      if(error) return  "Error in y(...):"+error.detail();
      auto _z=glFunctionPool.CreateFunction({"s","t","time"},dg.Text(2,1),error);
      if(error) return  "Error in z(...):"+error.detail();

      if(sutil::find_identifier(dg.Text(0,1),"time")!=dg.Text(0,1).end()||
         sutil::find_identifier(dg.Text(1,1),"time")!=dg.Text(1,1).end()||
         sutil::find_identifier(dg.Text(2,1),"time")!=dg.Text(2,1).end())
      {
...
void CMainWindow::m_2D_cartesian_dialog()
{
  ...
  ->std::string
  {
      expr::parse_error_t error;
      std::string str;
      auto floats=ParseFloats(dg,{1,2},str,glFunctionPool);
      if(!str.empty()) return str;
      if(floats[1]>=floats[2])return "min x must be less max x";
      auto f=glFunctionPool.CreateFunction({"x","time"},dg.Text(0,1),error);
      if(error) return "Error:"+error.detail();
      auto graph_1=CPlot2D::make_cartesian_dyn(f,{floats[1],floats[2]});
      graph_1.m_is_dynamic=sutil::find_identifier(dg.Text(0,1),"time")!=dg.Text(0,1).end();
...
void CMainWindow::m_2D_polar_dialog()
{
  ...
  {
      expr::parse_error_t error;
      auto f=glFunctionPool.CreateFunction({"phi","time"},dg.Text(0,1),error);
      if(error) return  "Error:"+error.detail();
...
void CMainWindow::m_2D_parametric_dialog()
{
  ...
  ->std::string
  {
      std::string str;
      expr::parse_error_t error;
      auto floats=ParseFloats(dg,{2,3},str,glFunctionPool);
      if(!str.empty()) return str;
      if(floats[2]>=floats[3])return "min s must be less max s";
      auto _x=glFunctionPool.CreateFunction({"s","time"},dg.Text(0,1),error);
      if(error) return  "Error in x(...):"+error.detail();
      auto _y=glFunctionPool.CreateFunction({"s","time"},dg.Text(1,1),error);
      if(error) return  "Error in y(...):"+error.detail();
...
{
  auto dg_ptr=std::make_unique<CEditConstantsWidget>(glFunctionPool);
...
{
  auto data=[&glFunctionPool](int r,int c)
  {
      if(c==0)      return glFunctionPool.BuildinConstant(r).Name();
      else if(c==1) return std::to_string(glFunctionPool.BuildinConstant(r).Value());
      else assert(false);
  };
  auto dg_ptr=std::make_unique<CInfoTableDialog>
              (CInfoTableDialog::ok_id,
               std::pair(glFunctionPool.BuildinConstants(),2),
               data);
  auto res=dg_ptr->exec();
...
{
  auto dg_ptr=std::make_unique<CEditFunctionsWidget>(glFunctionPool);
  auto res=dg_ptr->exec();
  assert(res==QDialog::Rejected);
}
...
void CMainWindow::m_BuiltInFunctions()
{
  ...
  {
      assert(c==0);
      return glFunctionPool.BuildinFunction(r).Name();
  };
  auto dg_ptr=std::make_unique<CInfoTableDialog>
              (CInfoTableDialog::ok_id,
               std::pair(glFunctionPool.BuildinFunctions( ),1),
               data);
  auto res=dg_ptr->exec();
  assert(res==QDialog::Accepted);
}
...
void CMainWindow::m_ContextMenuDialog()
{
  ...
  ->std::string
  {
      ...
      auto floats=ParseFloats(dg,{0,1,2,3},str,glFunctionPool);
      if(!str.empty()) return str;
...
CConstantTable::CConstantTable(CFunctionPool&pool):
m_pool(pool)
{
...
QVariant CConstantTable::data(const QModelIndex &index, int role) const
{
  ...
  if(i<m_pool.Constants( )&&role==Qt::DisplayRole)
    {
      if(j==0)
        {
          return QString::fromStdString(m_pool.Constant(i).Name());
        }
      else if(j==1)
        {
          return m_pool.Constant(i).Value();
        }
      else{}
    }
  return QVariant{};
...
{
  auto ins=m_pool.CreateConstant(str,val);
  if(!ins) return false;
  Repaint();
  return true;
}
...
{
  std::size_t r=mi.row();
  assert(r<m_pool.Constants());
  std::vector<CFunction> depend;
  m_pool.DependentFunctions(m_pool.Constant(r),depend);
  if(depend.empty())
  {
      bool b=m_pool.EraseConstant(m_pool.Constant(r));
      assert(b);
      Repaint();
      return;
  }
  auto dep_size=depend.size();
  auto data=[mdepend=std::move(depend)](int row,int col)
  {
      assert(col==0);
      return mdepend[row].Name();
  };
  ...
  {
      m_pool.EraseConstant(m_pool.Constant(r));
      Repaint();
  }
}

void CConstantTable::ResetRecord(const QModelIndex&mi,float vt)
{
  std::size_t r=mi.row();
  assert(r<m_pool.Constants( ));
  m_pool.Constant(r).SetValue(vt);
  Repaint();
}
...
void CConstantTable::EditButtonClicked()
{
  ...
  {
      std::string str;
      auto floats=ParseFloats(dg,{0},str,glFunctionPool);
      if(!str.empty()) return str;
      ResetRecord(ixs.front(),floats[0]);
      return {};
  };
  dg_ptr->SetText(0,1,std::to_string(m_pool.Constant(i).Value()));
  dg_ptr->SetPreprocessor(preprocess);
  dg_ptr->exec();

}

CEditConstantsWidget::CEditConstantsWidget(CFunctionPool&pool):
m_table(pool)
{
...
CFunctionsTable::CFunctionsTable(CFunctionPool&pool):
m_pool(pool)
{
}
...
{
  std::string res;
  for(const auto&arg:m_pool.Function(i).Args())
  {
      res+=arg;
      res+=",";
  }
...
QVariant CFunctionsTable::data(const QModelIndex &index, int role) const
{
  std::size_t i=index.row();
  std::size_t j=index.column();
  if(i<m_pool.Functions()&&role==Qt::DisplayRole)
  {
    if(j==0)
    {
         return QString::fromStdString(m_pool.Function(i).Name());
    }
    else if(j==1)
    {
    ...
    {
        return QString::fromStdString(m_pool.Function(i).Body());
    }
    else{}
  }
  return {};
...
expr::parse_error_t CFunctionsTable::ResetFunction(
                          int i,
                          const std::vector<std::string>&args,
                          const std::string&body)
{
  auto f=m_pool.Function(i);
  expr::parse_error_t per=m_pool.ReparseFunction(f,args,body);
  if(!per) Repaint();
  return per;
}
...
{
  int r=mi.row();
  auto fnc=m_pool.Function(r);
  std::vector<CFunction> depend;
  m_pool.DependentFunctions(fnc,depend);
  if(depend.empty())
  {
      m_pool.EraseFunction(fnc);
      Repaint();
      return;
  }
  //need check if plot use function...
  auto dep_size=depend.size();
  auto data=[mdep=std::move(depend)](int row,int col)
  {
      assert(col==0);
      return mdep[row].Name();
  };
  using info_t=CInfoTableDialog;
  auto dg=std::make_unique<info_t>
          (info_t::ok_id|info_t::cancel_id,
...
expr::parse_error_t CFunctionsTable::AddFunction(
                     const std::string&name,
                     const std::vector<std::string>&args,
                     const std::string&body)
{
  expr::parse_error_t error;
  auto f=m_pool.CreateAndRegisterFunction(name,args,body,error);
  if(!error) Repaint();
  return error;
}
...
void CFunctionsTable::AddButtonClicked()
{
  ...
  ->std::string
  {
      ...
      if(pe) return "Parse error:"+pe.detail();
      return {};
  };
  dg_ptr->SetPreprocessor(preprocess);
  dg_ptr->exec();
...
void CFunctionsTable::EditButtonClicked()
{
  ...
  std::vector<QString> v_str({"Name:","Args:","Body:"});
  auto dg_ptr=LinesEditDg(v_str);
  auto fnc=m_pool.Function(i);
  dg_ptr->SetText(0,1,fnc.Name());
  dg_ptr->Widget<QLineEdit>(0,1).setReadOnly(true);
  dg_ptr->SetText(1,1,MakeArgsString(i));
  dg_ptr->SetText(2,1,fnc.Body());
  auto preprocess=[this,fnc,i](const CBaseDialog&dg)
  ->std::string
  {
      std::vector<std::string> args;
      std::string mess=ParseArgs(dg.Text(1,1),args);
...
CEditFunctionsWidget::CEditFunctionsWidget(CFunctionPool&pool):
m_table(pool)
{
  m_view=new QTableView;
  m_table.SetView(m_view);
...
void CCalculateDialog::Calculate()
{
  ...
  expr::parse_error_t error;
  auto expr=glFunctionPool.CreateFunction({"x"},Text(0,1),error);
  if(error)
  {
      auto mess="Expression is not valid:"+error.detail();
      QMessageBox::information(nullptr,"Input error",QString::fromStdString(mess));
      return;
  }
  auto val=glFunctionPool.CreateFunction({},Text(1,1),error);
  if(error)
  {
      auto mess="Value is not valid:"+error.detail();
      QMessageBox::information(nullptr,"Input error",QString::fromStdString(mess));
      return;
  }
