...
class uniform_value<vec<T,dim>>:public uniform_value_base
{
    ...
    uniform_value&operator=(const range_t&rg)
    {
        ...
        else if constexpr(std::is_same_v<T,GLint>)
        {
            if constexpr(dim==2)
            {
                m_glUniform2i(rg[0],rg[1]);
            }
            else if constexpr(dim==3)
            {
                m_glUniform3i(rg[0],rg[1],rg[2]);
            }
            else if constexpr(dim==4)
            {
                m_glUniform4i(rg[0],rg[1],rg[2],rg[3]);
            }
        }
        else if constexpr(std::is_same_v<T,GLfloat>)
        {
            if constexpr(dim==2)
            {
                m_glUniform2f(rg[0],rg[1]);
            }
            else if constexpr(dim==3)
            {
                m_glUniform3f(rg[0],rg[1],rg[2]);
            }
            else if constexpr(dim==4)
            {
                m_glUniform4f(rg[0],rg[1],rg[2],rg[3]);
            }
        }
        else if constexpr(std::is_same_v<T,GLdouble>)
        {
            if constexpr(dim==2)
            {
                m_glUniform2d(rg[0],rg[1]);
            }
            else if constexpr(dim==3)
            {
                m_glUniform3d(rg[0],rg[1],rg[2]);
            }
            else if constexpr(dim==4)
            {
                m_glUniform4d(rg[0],rg[1],rg[2],rg[3]);
            }
        }
        glCheckError();
        return *this;
