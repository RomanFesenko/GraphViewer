...
using point_t=Point2D<float>;
using point3D_t=Point3D<float>;
using matrix3D_t=std::array<std::array<float,4>,4>;

const point3D_t glZeroVec={0.0f,0.0f,0.0f};

const point3D_t glXOrt={1.0f,0.0f,0.0f};
const point3D_t glYOrt={0.0f,1.0f,0.0f};
const point3D_t glZOrt={0.0f,0.0f,1.0f};


const matrix3D_t glIdentityMx={std::array<float,4>{1.0f,0.0f,0.0f,0.0f},
...
inline void SetMatrix(const matrix3D_t& matrix,GLenum en_matr)
{
    ...
    //при использовании  =_buffer в лямбду копируется
    // массив а не указатель
    columns_visit([&_buffer,&matrix,i_buff](size_t i,size_t j)mutable
    {
        _buffer[i_buff++]=matrix[i][j];
    },4,4);

    glLoadMatrixf(_buffer);
}
